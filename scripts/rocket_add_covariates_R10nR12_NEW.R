...
cohort_eps <- trial_cohort %>% 
  ...
  # take only the last endpoint occurence per ID
...
# Occurence of covariate endpoints in cohorts
pop_desc <- cohort_eps %>%
  # filter for the endpoints of interest
  filter(ENDPOINT %in% covariates$covariates) %>%
  # group by patient ID and endpoint
  group_by(FINNGENID, ENDPOINT) %>%
  # summarize the age and time of endpoint for each endpoint of interest (last time if it occured multiple times)
  ...
  pivot_wider(names_from = ENDPOINT, values_from = endpoint)

results <- data.frame(
  trial_cohort[,1:2] 
)

# Resulting dataframe with occurence of each endpoint covariate for each ID
...
cohort_meds <- trial_cohort %>% left_join(meds) %>%
  ...
  # take only the last medication occurence per ID
...
# Occurence of covariate medication in cohorts
...
# Adding medication covariates to the resulting datafram
