...
disc <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter(EVENT_AGE >= age) %>% 
  # filter for Apixaban and Warfarin
  filter(ATC == "B01AF01" | ATC == "B01AA03") %>%
  # keep only the medication of group assignment in case someone switches later
  filter((group == 1 & ATC == "B01AF01") | (group == 0 & ATC == "B01AA03"))  %>%  
  # REMOVE VNRO CODES THAT DON'T HAVE ANY MAPPING CURRENTLY:
...
switch <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter(str_detect(ATC, "^B01AF01|^B01AA03")) %>%
  # change the medication assignment -> SWITCHES/IDs with two meds   
  filter((group == 1 & ATC == "B01AA03") | (group == 0 & grepl("^B01AF01", ATC))) %>% 
  # select last purchase
  group_by(FINNGENID) %>% 
  arrange(EVENT_AGE) %>% 
...
##### End of trial ####
# end of trial up to 4 years
trial_cohort <- trial_cohort %>% 
  mutate(
    age_endoftrial = age + 4.0,
    survival_years_endoftrial = 4.0
  )
...
trial_cohort <- trial_cohort %>% mutate(primary_outcome = if_else(abs(time - round(survival_years_po, 2)) > 0.1 & !(time > 4)  | is.na(survival_years_po), 0, primary_outcome))
