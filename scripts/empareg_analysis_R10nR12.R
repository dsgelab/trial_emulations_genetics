...
disc_tmp   <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter(str_detect(ATC, "^A10BK03|^A10BH")) %>%
  # keep only the medication of group assignment in case someone switches later
  filter((group == 1 & ATC == "A10BK03") | (group == 0 & grepl("^A10BH", ATC))) %>% 
...
switch <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter((group == 1 & grepl("^A10BH", ATC)) | (group == 0 & ATC == "A10BK03")) %>% 
  # select last purchase
  group_by(FINNGENID) %>% 
  arrange(EVENT_AGE) %>% 
...
##### End of trial ####
# end of trial up to 4.6 years
trial_cohort <- trial_cohort %>% 
  mutate(
    age_endoftrial = age + 4.6,
    survival_years_endoftrial = 4.6
  )
...
trial_cohort <- trial_cohort %>% mutate(primary_outcome = if_else(abs(time - round(survival_years_po, 2)) > 0.1 & !(time > 4.6)  | is.na(survival_years_po), 0, primary_outcome))
