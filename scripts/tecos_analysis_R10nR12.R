...
meds <- fread(file2) %>% filter(FINNGENID %in% trial_cohort$FINNGENID); gc()
mp <- fread(file3) %>% filter(FINNGENID %in% trial_cohort$FINNGENID); gc()
vnro_dict <- fread(file4)
covariates <- fread(file5)
...
##### Primary Outcomes ####
# 3P MACE
po <- "I9_K_CARDIAC|C_STROKE|I9_MI|I9_CHD"
cohort_po1 <- cohort_endpoints %>% filter(grepl(po, ENDPOINT)) %>% arrange(FINNGENID, EVENT_AGE)
cohort_po1 <- cohort_po1[match(unique(cohort_po1$FINNGENID), cohort_po1$FINNGENID)]
cohort_po1$primary_outcome <- 1
cohort_po1 <- cohort_po1 %>% select(FINNGENID, primary_outcome, primary_outcome_age = EVENT_AGE)

# Died from a cardiovascular cause
cohort_po2 <- cohort_endpoints %>% filter(EVENT_TYPE == "DEATH" & grepl("I9_", ENDPOINT)) %>% 
  group_by(FINNGENID) %>% slice(1) %>% 
  mutate(primary_outcome = 1) %>% select(FINNGENID, primary_outcome, primary_outcome_age = EVENT_AGE)

cohort_po <- rbind(cohort_po1, cohort_po2) %>% group_by(FINNGENID) %>% arrange(primary_outcome_age) %>% slice(1)
...
trial_cohort <- trial_cohort %>% mutate(survival_years_po = primary_outcome_age - age) 
...
##### End of registry ####
# End of registry day
eor <- max(long_ep$APPROX_EVENT_DAY)

# get age for end of the registry:
...
disc_tmp   <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter(str_detect(ATC, "^A10BH01|^A10BB01|^A10BB07|^A10BB09|^A10BB12")) %>% 
  # keep only the medication of group assignment in case someone switches later
  filter((group == 1 & ATC == "A10BH01") | (group == 0 & grepl("^A10BB01|^A10BB07|^A10BB09|^A10BB12", ATC))) %>% 
...
switch <- trial_cohort %>% select(FINNGENID, group, age) %>% 
  ...
  filter((group == 0 & ATC == "A10BH01") | (group == 1 & str_detect(ATC, "^A10BB01|^A10BB07|^A10BB09|^A10BB12"))) %>% 
  # select last purchase
  group_by(FINNGENID) %>% 
  arrange(EVENT_AGE) %>% 
...
trial_cohort <- trial_cohort %>% 
  mutate(
    age_endoftrial = age + 5.0,
    survival_years_endoftrial = 5.0
  )


trial_cohort %>% select(tidyselect::starts_with("survival"))

##### Survival time (as incident which has occurred the first) #####
...
# primary outcomes per group BEFORE survival analysis adjustment 
trial_cohort %>% group_by(group) %>% summarize(count_po = sum(primary_outcome == 1))
trial_cohort <- trial_cohort %>% mutate(primary_outcome = if_else(abs(time - round(survival_years_po, 2)) > 0.1 & !(time > 5)  | is.na(survival_years_po), 0, primary_outcome))
